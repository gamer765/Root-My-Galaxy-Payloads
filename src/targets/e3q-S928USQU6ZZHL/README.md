...
- `kernelsu/ksud-e3q-S928USQU6ZZHL-kdp-ksun-3.3.0` — AArch64 KernelSU Next 3.3.0 userspace daemon with synchronous late-load and the matching module embedded.
...
fingerprint are ZZHL-specific. The corrected KernelSU pair is CI-verified;
device validation of that pair remains pending. No boot/recovery image is
included and nothing here should be flashed directly.
...
image. The module and daemon were both built from KernelSU Next source commit
`3b18216f71df189ab3d1b1ce0bdb21be1268e771` with the Samsung KDP/RKP/DEFEX
compatibility port. The module has an empty `__versions` section and is
intended for the project’s manual-relocation late-load path.
...
late-load/staging patch. It is 3,764,280 bytes with SHA-256
`a87c3f334e8feede359f16d63e6503312a18b403203ce54b02de71d250e7fa7f`; its
embedded module matches the standalone ZZHL module byte-for-byte
