...
| `e3q-S928USQU6ZZHL` | Galaxy S24 Ultra `SM-S928U1` | `6.1.162` | ZZHL legacy S928 app + KernelSU Next 3.3.0; hardware retest pending |

The ZZHL row is the Android 17 beta build whose app payload uses the legacy
S928 engine and whose KernelSU payload uses KernelSU Next 3.3.0 with the
synchronous late-load path. It uses the `USQU6ZZHL` release string observed
in the supplied vendor boot image. The active pair embeds the legacy S928 DZG1
KernelSU Next module shape with a same-length ZZHL vermagic retarget; CI has
verified the pairing and static invariants, while device validation remains
pending.
